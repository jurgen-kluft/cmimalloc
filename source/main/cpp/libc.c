...
#include "cmimalloc/private/prim.h" // mi_prim_getenv
...
void _mi_vsnprintf(char* buf, size_t bufsize, const char* fmt, va_list args)
{
    ...
    while (true)
    {
        ...
        {
            if ((c >= ' ' && c <= '~') || c == '\n' || c == '\r' || c == '\t')
            { // output visible ascii or standard control only
                mi_outc(c, &out, end);
            }
        }
        else
        {
            ...
            else if (c == 'p' || c == 'x' || c == 'u')
            {
                ...
                if (c == 'x' || c == 'u')
                {
                    if (numtype == 'z')
                        ...
                        x = (uintptr_t)va_arg(args, unsigned long long);
                    else
            ...
            else if (c == 'i' || c == 'd')
            {
                ...
                if (numtype == 'z')
                    ...
                    x = (intptr_t)va_arg(args, long long);
                else
                    x = va_arg(args, long);
                char pre = 0;
                if (x < 0)
